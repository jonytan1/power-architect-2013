...
public class SQLSchema extends SQLObject {
	...
	public boolean removeTable(SQLTable table) {
		...
		if (index != -1) {
			 tables.remove(index);
			 if (!table.isMovingToAnotherSchema()) {
	...
	public void addTable(SQLTable table, int index) {
	    ...
	    // Ignore this add operation generated by table's "setSchemaParent" when to undo or to re-do.
	    if (!table.isMagicEnabled() && !isMagicEnabled()) return;
