 ...
 * <code>
 *         IDatabaseMeta dm = DatabaseMetaFactory.creator(dbmd.getDatabaseProductName());
 *     return dm.fetchColumnsWithRemark(catalog, schema, table, dbmd);
 * </code>
 * 
...
public class DatabaseMetaFactory {
    ...
    public static DatabaseMeta creator( String databaseProductName ){
        String key = null;
        ...
        try {
            if ( key != null ){
                ...
                return (DatabaseMeta)c.newInstance();
            }
            ...
            logger.error("Fail to create DatabaseMeta class for \"" + key + "\":" + e.toString() );
        }
